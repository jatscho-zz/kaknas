    ...
    <template v-for="(item, itemIndex) in tableData">
          ...
          <template v-if="field.visible">
            ...
            <template v-else>
              <td v-if="hasCallback(field)"
                :style="checkColor(itemIndex, item, field)"
                :key="fieldIndex"
                :class="field.dataClass"
                v-html="callCallback(field, item)"
                @click="onCellClicked(item, field, $event, tableFields)"
                @dblclick="onCellDoubleClicked(item, field, $event)"
                @contextmenu="onCellRightClicked(item, field, $event)"
              ></td>
              <td v-else
      ...
      <template v-if="useDetailRow">
        <transition :name="detailRowTransition" :key="itemIndex">
          ...
          >
            <td :colspan="countVisibleFields">
              <component :is="detailRowComponent" :commit-data="commitData" :row-data="item" :row-index="itemIndex"></component>
            </td>
          </tr>
        </transition>
      </template>
...
export default {
  ...
  mounted () {
    ...
    //this.compareGitRefs()
    // make a call to http://localhost:5000/diff-status and save result in a var
    // we now have a dictionary of diffs
    // loop through both dictionaries and compare each row. If there is a diff, we append some sort of value to it to
    // indicate that it should be highlighted red
  ...
  methods: {
    ...
    loadData (sortOrder) {
      ...
      return this.fetch(this.apiUrl, this.httpOptions)
            .then((response) => {
                this.firstres = response
                return axios.get(this.diffUrl).then(response => {
                  this.secondres = response
                }).catch(() => this.loadDiffFailed('error'))
            }).catch(() => this.loadModulesFailed('error'))
            .then((response) => {
                return axios.get(this.commitsUrl).then(response => {
                  this.commitData = response.data.all_subpath_commits
                }).catch(() => this.loadCommitsFailed('error'))
            }).catch(() => this.loadModulesFailed('error'))
            .then((response) => {
              this.loadSuccessDiff(this.firstres, this.secondres)
              this.loadSuccessModules(this.firstres)
            }).catch(() => this.loadDiffFailed('error'))
    },
    fetch (url, httpOptions) {
      return this.httpFetch
          ? this.httpFetch(url, httpOptions)
          : axios[this.httpMethod](url, httpOptions)
    },
    loadSuccessModules (res) {
      this.fireEvent('load-success', res)

      // this method allows us to sort based on module path
      let body = this.transform(res.data)
      let sorted_keys = res.data.sorted_modules_keys[0]
      let reverse_keys = res.data.reversed_modules_keys[0]
      ...
      this.$nextTick(function() {
        this.fixHeader()
        this.fireEvent('pagination-data', this.tablePagination)
        // this.fetch(this.diffUrl, this.httpOptions).then((response) => {
        //     this.loadSuccessDiff(res, response),
        // }).catch(() => this.loadDiffFailed(response))
        this.fireEvent('loaded')
      })
    },
    loadSuccessDiff (res, response) {
      const modulesInfo = res.data.data // this is an array
      const sortOrder = res.data.sortOrder
      const diffInfo = response.data  // this is a dict
    ...
    checkColor (itemIndex, item, field) {
      // highlight table cell red if project's git ref differs from Greenfield
      for (var key in item) {
    ...
    onCellClicked (dataItem, field, event, tableFields) {
